...
```bash
getData$ sam build --use-container
```
...
```bash
getData$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
getData$ sam logs -n HelloWorldFunction --stack-name getData --tail
```
...
```bash
getData$ pip install pytest pytest-mock --user
getData$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name getData
```
